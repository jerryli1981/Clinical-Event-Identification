...
from utils import read_sequence_dataset_onehot, iterate_minibatches_,loadWord2VecMap, generateTestInput_phase2

def event_dr_classifier(args, input_var, target_var, wordEmbeddings, seqlen, num_feats):
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if not os.path.exists(model_save_path+".dr"):
            break
        fileIdx += 1
    ...
    if args.mode == "train":
        ...
        for epoch in range(args.epochs):
            ...
            for i, batch in enumerate(iterate_minibatches_((X_train, Y_labels_train), args.minibatch, shuffle=True)):
                ...
                inputs, labels= batch
                train_loss += train_fn(inputs, labels[:,2:7])
                train_batches += 1
            ...
            for i, batch in enumerate(iterate_minibatches_((X_dev, Y_labels_dev), args.minibatch, shuffle=True)):
                ...
                loss, acc = val_fn(inputs, labels[:,2:7])
                val_acc += acc
                val_loss += loss
            ...
            if best_val_acc < val_score:
                ...
                print "Saving model......"
                save_network(model_save_path+".dr",get_all_param_values(network))
                ...
                output_dir = os.path.join(base_dir, 'uta-output-dr-validate')
                ...
                for dir_path, dir_names, file_names in os.walk(input_text_dev_dir):
                    ...
                    for i, fn in enumerate(file_names):
                        ...
                        spans, features = generateTestInput_phase2(data_dir, input_text_dev_dir, input_ann_dev_dir, fn, window_size, num_feats)
                ...
                os.system("python -m anafora.evaluate -r annotation/coloncancer/Dev/ -p uta-output-dr-validate/")
        ...
        _, _, network = event_dr_classifier(args, input_var, target_var, wordEmbeddings, seqlen, num_feats)

        print model_save_pre_path
        saved_params = load_network(model_save_pre_path+".dr")
        set_all_param_values(network, saved_params)

        pred_fn = theano.function([input_var], T.argmax(get_output(network, deterministic=True), axis=1))

        
        ann_dir = os.path.join(base_dir, 'thymedata-1.2.0-coloncancer-test-event-time/coloncancer')
        output_dir = os.path.join(base_dir, 'uta-output-dr')
